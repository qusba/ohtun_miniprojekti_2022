...
class App:
    ...
    def list_references(self):
        while True:
            self.io.write(
                "Syötä 1 listataksesi viitteet lisäysjärjestyksessä vanhin ensin")
            self.io.write(
                "Syötä 2 listataksesi viitteet lisäysjärjestyksessä uusin ensin")
            self.io.write(
                "Syötä 3 listataksesi viitteet aakkosjärjestykseen kirjoittajan sukunimen mukaan")
            self.io.write(
                "Syötä 4 listataksesi viitteet käänteiseen aakkosjärjestykseen kirjoittajan sukunimen mukaan")
            ...
            if command == "1":
                try:
                    ...
                    self.referenceHandler.print_references(
                        self.fileHandler.read_book_refs_from_file())
                    self.io.write("")
            ...
            if command == "2":
                try:
                    self.io.write("")
                    self.referenceHandler.print_references(
                        self.referenceHandler.reverse_the_reference_list())
                    self.io.write("")
                except Exception as error:
                    self.io.write("\nJotain meni vikaan \n")
            if command == "3":
        ...
        while True:
            ...
            if command == "1":
                ...
                try:
                    ...
                    while True:
                        tag = self.io.read(
                            "Tägi? (Paina enter viimeistelläksesi viitteen luonti): ")
                        if not tag:
                            break
                        tags.append(tag)
                ...
                try:
                    return_bit = self.fileHandler.write_bibtext_file(
                        self.bib_file_path)
                    if return_bit == 0:
                        self.io.write("Ei viitteitä \n")
                    else:
                        self.io.write(
                            "references.bib tiedosto luotu sijaintiin "+str(self.cwd)+"\n")
                except Exception as error:
                    self.io.write("\nJotain meni vikaan \n")
                ...
                try:
                    ...
                    rem_success = self.fileHandler.remove_reference_from_file(
                        key)
                ...
                try:
                    key = self.io.read(
                        "Anna viitteen avain minkä haluat tägätä: ")
                    references = self.referenceHandler.get_references()
                    ...
                    if key not in keys:
                        ...
                        continue
                    ref_to_tag = next(
                        ref for ref in references if ref.get_key() == key)
